---
title: "主从复制是怎么实现的？"
source: "https://www.xiaolincoding.com/redis/cluster/master_slave_replication.html"
author:
  - "[[小林coding]]"
published: undefined
created: 2026-09-18
...
repl\_backlog\_buffer 最小的大小可以根据下面这个公式估算。
...
那么 repl\_backlog\_buffer 大小就不能低于 5 MB，否则新写的命令就会覆盖旧数据了。
...
- Redis 的 INFO replication 命令可以查看主节点接收写命令的进度信息（master\_repl\_offset）和从节点复制写命令的进度信息（slave\_repl\_offset），所以，我们就可以开发一个监控程序，先用 INFO replication 命令查到主、从节点的进度，然后，我们用 master\_repl\_offset 减去 slave\_repl\_offset，这样就能得到从节点和主节点间的复制进度差值了。
