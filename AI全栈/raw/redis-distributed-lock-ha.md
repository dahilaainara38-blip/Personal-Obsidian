---
title: "如何保证 Redis 分布式锁的高可用和高性能？"
source: "https://www.xiaolincoding.com/redis/cluster/redlock.html"
author:
  - "[[小林coding]]"
published: undefined
created: 2026-09-18
...
分布式锁和分布式事务，堪称分布式系统中的两大“拦路虎”，理论深奥，实践中又极易出错。在面试中，分布式锁更是高阶岗位的必考题。然而，还是有很多同学对分布式锁的理解仅仅停留在 `SETNX` 和“设置一个过期时间”的表层。当被进一步追问，比如“如何优雅地等待锁？”、“加锁超时了怎么办？”、“业务执行时间超过了锁的过期时间又该如何？”时，就开始不知道怎么回答了。
...
![1](https://cdn.xiaolincoding.com//picgo/aea8c59e27667257526dd0abcd3b5e05.png)
...
![2](https://cdn.xiaolincoding.com//picgo/0b7e2861cac7060aefe73ff5557313eb.png)
...
![3](https://cdn.xiaolincoding.com//picgo/54c94a862fc00a12e561511418f32ad1.png)

- **情况B：GET 返回 uuid-client-A。**

这说明上一次的 SETNX 执行成功了！只是响应包在路上丢了。此时客户端已经持有了锁，它需要做的只是重置一下这把锁的过期时间（我们稍后会讲到），然后心满意足地返回“加锁成功”。

![4](https://cdn.xiaolincoding.com//picgo/8ddc69be0a46b8001625d18197f06d15.png)

- **情况C：GET 返回 uuid-client-B。**

这说明在客户端A超时和重试的间隙，锁被客户端B拿走了（可能是A上次加锁失败，B加锁成功）。此时客户端A重试失败，应进入上述的等待逻辑。

![5](https://cdn.xiaolincoding.com//picgo/77242e56fdc66a5fc36d6aa7559778b4.png)
...
![6](https://cdn.xiaolincoding.com//picgo/5eba47fc4a060f98cbf3f653d3588626.png)
...
![7](https://cdn.xiaolincoding.com//picgo/8ec608f083ea3442bedb29864f445515.png)
...
![8](https://cdn.xiaolincoding.com//picgo/d5955486ffae03b607bd4febe8dc4c78.png)
...
![9](https://cdn.xiaolincoding.com//picgo/d8e2c86c379986397e60eac5fd1dae0a.png)
...
![10](https://cdn.xiaolincoding.com//picgo/62f1bb6eb5d95040f9125d3966d18e92.png)
...
![11](https://cdn.xiaolincoding.com//picgo/c1a0b3514559203d5bbac166e3974397.png)
...
![12](https://cdn.xiaolincoding.com//picgo/f6c62f3e30c1c00976606c4b1ee2bf84.png)
...
![13](https://cdn.xiaolincoding.com//picgo/a874475442d46396e48cd89e5267d687.png)
...
#### Redlock 的争议：Martin Kleppmann 与 antirez 之争

值得一提的是，Redlock 发布之后曾经引发过一场非常有名的争论。2016 年，分布式系统专家 **Martin Kleppmann** （《Designing Data-Intensive Applications》的作者）在博客 [*How to do distributed locking*](https://martin.kleppmann.com/2016/02/08/how-to-do-distributed-locking.html) 中对 Redlock 的安全性提出了几点质疑：

1. **时钟假设不可靠** ：Redlock 依赖"各节点时钟以相近速度向前走"来判断锁是否过期，而真实系统中时钟可能因为 NTP 校准、管理员手动调整等原因发生 **跳变** ，导致同一把锁在一方看来仍然持有、在另一方看来却已经过期。
2. **进程暂停（GC / Swap / 缺页）** ：客户端在拿到锁之后，可能因为 Stop-The-World GC、被操作系统调度出去等原因暂停几十秒甚至更久。等它恢复的时候锁早已过期，但客户端自己 **并不知道** ，于是拿着一把"已经过期的锁"继续写入共享资源，从而破坏互斥性。
3. **网络延迟** ：消息在网络中排队、延迟送达，同样会让客户端在"自以为仍持有锁"的情况下继续操作已经被别人接管的资源。

Martin 的核心观点是：只要锁的正确性依赖"时间"，那么任何基于时钟的分布式锁就都无法提供严格的互斥，必须由 **被保护的资源本身** 配合单调递增的 **fencing token** （版本号校验），丢弃所有"迟到"的请求，才是真正安全的做法。

Redis 作者 **antirez** 随后发表了 [*Is Redlock safe?*](http://antirez.com/news/101) 进行回应，认为 Martin 的分析混淆了不同的故障模型：在绝大多数实际场景下，Redlock 对时钟漂移的要求是合理的；而且 fencing token 本身也需要对资源服务器做额外改造，并不是银弹。

这场争论直到今天也没有一个公认的结论。生产实践中的建议是：

- 如果业务 **对锁的正确性要求极高** （例如涉及资金扣款、库存强一致等场景），应当选择具备强一致性的协调服务（如 **ZooKeeper、etcd** ），并配合 fencing token 做最终的幂等校验；
- 如果分布式锁只是用于 **提升性能、去重、避免重复执行** 这类"尽力而为"的互斥，单实例 Redis 锁或 Redlock 通常已经足够。
...
![14](https://cdn.xiaolincoding.com//picgo/57999ca081b6e5ef77eec2a4dcbd5173.png)
...
![15](https://cdn.xiaolincoding.com//picgo/f66a3d03961e5fa899bb57582ccc1064.png)
...
![16](https://cdn.xiaolincoding.com//picgo/74a04ffca054bea449c632ff6932af3a.png)

## 4\. 总结

从最初的 `SETNX` 到完善的续约、原子化释放、再到 Redlock 以及各种性能优化手段，我们可以看到，分布式锁的核心始终围绕两个关键词展开： **可靠性** 与 **性能** 。

可靠性确保锁语义不被破坏，性能则决定方案能否真正落地。工程实践中，锁并非万能，很多场景完全可以用乐观锁或一致性哈希等思路替代，从根源上消除“锁”的需求。

掌握这些设计背后的取舍逻辑，远比死记实现细节更重要。

分布式锁不是终点，而是理解分布式一致性与系统取舍的起点。
