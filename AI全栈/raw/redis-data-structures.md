...
然后在 Redis 5.0 设计了新的数据结构 listpack，沿用了压缩列表紧凑型的内存布局，并在 **Redis 7.0 版本** 正式将 Hash 对象和 Zset 对象的底层数据结构实现之一的压缩列表，替换成由 listpack 实现。
...
可以看到，保存元素的容器是一个 contents 数组，虽然 contents 被声明为 int8\_t 类型的数组，但是实际上 contents 数组并不保存任何 int8\_t 类型的元素，contents 数组的真正类型取决于 intset 结构体里的 encoding 属性的值。比如：
...
- **从内存占用上来比较，跳表可以通过参数 p 调整层数开销，但不能只凭前向指针数量断言它比平衡树更省内存** 。忽略最大层数的截断时，跳表节点的平均层数为 1/(1-p)，也就是平均前向指针数。Redis 取 p=1/4，因此平均每个节点约有 1.33 个前向指针；此外，每个节点还有 1 个后向指针 `backward` ，用于连接跳表节点的指针合计平均约 2.33 个。实际内存开销还要计入每层的跨度 `span` 、节点的数据字段和内存对齐等；平衡树也可能有父指针、平衡信息等额外字段，需要按具体实现比较。
...
文章说到set也会使用hashtable来实现

这个是hashtable每个key-value的结构体。set是字符串的集合，set本质上只存放1个字符串到 dictEntry 。其中问了gpt说，会存放到void \*key中。那么void \*val 就是根本没有用上？那这样set中的每个字符串不就浪费了8个字节（64位机器的指针大小）？
