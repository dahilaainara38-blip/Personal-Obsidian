---
title: "多节点争抢资源， Redis 分布式锁是怎么实现的？"
source: "https://www.xiaolincoding.com/redis/module/setnx.html"
author:
  - "[[小林coding]]"
published: undefined
created: 2026-09-18
...
tags:
  - "clippings"
---
> 原文地址： [面试官：多节点争抢资源， Redis 分布式锁是怎么实现的？ (opens new window)](https://mp.weixin.qq.com/s/SXDJpOphGTy1NLeEAL9FbA)

大家好，今天给大家分享 Redis 里的一个知识点 — Redis 分布式锁。

**在分布式系统的世界里，Redis 就像一座高效运转的共享仓库** — 多个节点通过共享数据协同发力，左手接过业务系统的海量读写请求，右手以毫秒级速度完成数据交付。

![图片](https://cdn.xiaolincoding.com//picgo/9c582e7a3248b6f5a1034798e24d3591.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/0f22fbdbacef9be16e54fce6671963f3.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/c29312496697bcd68655c53d95b07cd4.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/255120e2e6870fd3d4551e0e33821245.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/189c227866aa30025e61a4177e90b01b.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/378a612b590c4bcbf1dddec5c08ba29c.png)
...
Redis 单节点锁的核心命令看似简单：\*\*SET lock\_key unique\_value NX EX 30。\*\*但这条命令里藏着解决两大基础难题的巧思。我们逐一拆开来看：

先看 unique\_value，它是给每个节点生成的唯一标识，就像给每个节点发了一张专属入场券。释放锁的时候，节点必须出示这张入场券，只有校验通过，才能删除自己持有的锁，绝不会误删别人的锁。

![图片](https://cdn.xiaolincoding.com//picgo/197b6d070cad14bfdc79f8801b4a3901.png)

再看 NX（Not eXists），它的意思是仅当锁对应的 lock\_key 不存在时，才执行加锁操作。这就像只有空座位才能入座，直接保证了锁的互斥性 — 同一时间，只有一个节点能成功获取锁，从根源上解决了大家抢着用资源的锁争抢问题。

![图片](https://cdn.xiaolincoding.com//picgo/3c8513393b81b26b34bde2145f14819e.png)

然后是 EX 30，它的作用是给锁设置 30 秒的过期时间。好比座位有最长占用时限，超时后自动 “让座”，不会一直占着资源不释放，从根本上避免了节点宕机等异常情况导致的“僵尸锁”。

![图片](https://cdn.xiaolincoding.com//picgo/c32b7b54660bd71b1c3fd1af54723b83.png)

而这条 SET 命令的关键，就是把抢锁（NX）和设过期时间（EX），变成了一步完成的原子操作，中间没有任何可中断的间隙：

**要么指令未执行，锁没抢到；要么指令完整执行，抢到的锁已经自带过期时间。**

![图片](https://cdn.xiaolincoding.com//picgo/32f40eaf0ed471ac2b4dd71b260af089.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/9e87f58ffa425d88ac24191640d1aeee.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/27c2884bd4b5e9124624aac42f06b402.png)

通过 Redlock 算法的加锁机制，我们确保了分布式环境下资源的互斥访问，但锁的释放环节同样需要谨慎处理，毕竟一旦释放逻辑出现疏漏，就可能引发新的并发混乱。

### 2、释放锁机制：先 “验身份” 再删锁

先前加锁时，我们会针对要保护的共享资源，在锁节点存入一把 “带身份标识的锁” — unique\_value，就像给这把锁刻上了自己的 “专属名字”。等业务要释放共享资源时，不能直接删锁，得先去锁节点验证：
...
![图片](https://cdn.xiaolincoding.com//picgo/6e2d4f8049a1a7c68cb90081893db9fa.png)

比如你刚查完，确认这把锁是自己的，可还没来得及删，这把锁的过期时间就到了，系统自动把它释放了；而就在这一瞬间，其他业务刚好抢到了这把锁。等你反应过来执行删除操作时，删的已经不是自己的锁，而是别人刚抢到的新锁了。

![图片](https://cdn.xiaolincoding.com//picgo/ffc089713415eea2c715bfb122b09022.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/20c1fa4ebfd15e0109b13c12f744e557.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/7ed7e09a36338dcd0d47c48740e76342.png)

使用时需要注意三个关键细节，不然容易出问题：

**第一个细节是 unique\_value 必须独一无二。**

命令里的 "node1\_uuid\_12345" 可不是随便填的字符串，它是每个加锁请求的专属 “身份证”，得保证全网唯一。核心目的就是让后续解锁时，能准确识别出 “这把锁是我加的”，避免误解锁别人的锁。

**第二个是过期时间 `EX 30` 的设置，得合理预留缓冲。**

比如你预估一个任务平均 10 秒能完成，那过期时间就别设 10 秒，建议设 30 秒。多出来的 20 秒是缓冲，万一某个任务因为网络延迟、数据量稍大慢了点，也不会出现 “任务还没做完，锁就过期了” 的情况。

**第三个是加锁失败后别做无效等待。**

如果执行上面的 SET 命令后，返回的是 `nil，说明锁已经被别人占用了。这时候别一直死等，要么设置最多重试 3 次（每次间隔几百毫秒），要么直接返回 “操作失败，请稍后再试”，避免浪费服务器资源。`

![图片](https://cdn.xiaolincoding.com//picgo/519c0d7f1f6b04283f11f25990c501ee.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/a2b7d1c6f8f782d3a6a3850f9e3754eb.png)

目前主流的开源框架 Redisson 已经内置了这个看门狗机制，使用起来十分便捷。

不过要注意，不管是基础版单节点锁，还是带看门狗的锁，都只是解决了单节点场景的锁问题。如果是支付、下单这类核心业务，我们需要更高的可用性保障，这时候基于 **Redlock 算法** 的高可用部署方案就成了首选。

### 3、核心场景：高可用锁守住业务“生命线”

针对支付、下单等核心业务场景，建议直接采用 Redlock 算法，并部署至少 3 个独立的 Redis 锁节点。具体部署建议如下：

![图片](https://cdn.xiaolincoding.com//picgo/774683165caadc7a17bc69c1f33f9c8b.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/04a676df94b14e5378a9133e30d76582.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/414581b5a2d656f58186c82ba9f721b6.png)
...
![图片](https://cdn.xiaolincoding.com//picgo/17cc9d9d768faac8628a38c4f6cab4ed.png)
...
← [Redis 过期删除策略和内存淘汰策略有什么区别？](https://www.xiaolincoding.com/redis/module/strategy.html) [主从复制是怎么实现的？](https://www.xiaolincoding.com/redis/cluster/master_slave_replication.html) →

[![dahilaainara38-blip](https://avatars.githubusercontent.com/u/276061814?u=63f4a995af968d710f503de436ce5edfdcf86eb7&v=4)](https://github.com/dahilaainara38-blip "dahilaainara38-blip")
